...
mkdir -p "$HOME"/"$xdg_replacement_dir" # create a directory to move the XDG directories to, so that they do not bloat the /home/[user]

mkdir -p "$HOME"/"$xdg_replacement_dir"/Templates "$HOME"/"$xdg_replacement_dir"/Pictures # for XDG directories that aren't useless in functionality, but still bloat up the home directory, create replacement directories, but put them into a hidden(.) directory instead of the root of home directory(Templates directory is needed so that files could be created from the file manager, Pictures directory is needed so that screenshots aren't thrown into the home directory)
